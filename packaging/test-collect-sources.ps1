...
foreach ($name in @('Recipe-Matches', 'Copy-Source', 'Copy-Tree', 'Verify')) {
...
try {
    ...
    New-Item -ItemType Directory "$fixture/udis/scripts/__pycache__" -Force | Out-Null
    Set-Content "$fixture/udis/scripts/ud_opcode.py" 'preferred source'
    Set-Content "$fixture/udis/scripts/__pycache__/ud_opcode.cpython-312.pyc" 'generated cache'
    $omitted = @(Copy-Tree "$fixture/udis" "$fixture/collected" -OmitUdisBytecode)
    if ($omitted.Count -ne 1 -or $omitted[0].path -ne 'scripts/__pycache__/ud_opcode.cpython-312.pyc') { throw 'Generated cache exclusion not recorded' }
    if (-not (Test-Path "$fixture/collected/scripts/ud_opcode.py")) { throw 'Preferred Python source omitted' }
    if (Test-Path "$fixture/collected/scripts/__pycache__/ud_opcode.cpython-312.pyc") { throw 'Generated bytecode copied' }
    Verify "$fixture/collected/scripts/ud_opcode.py" SHA256 $omitted[0].sourceSha256
    $rejected = $false
    try { Copy-Tree "$fixture/udis" "$fixture/not-udis" } catch { $rejected = $true }
    if (-not $rejected) { throw 'Bytecode accepted outside reviewed port' }
    Remove-Item "$fixture/udis/scripts/ud_opcode.py"
    $rejected = $false
    try { Copy-Tree "$fixture/udis" "$fixture/no-source" -OmitUdisBytecode } catch { $rejected = $true }
    if (-not $rejected) { throw 'Bytecode omitted without preferred source' }
